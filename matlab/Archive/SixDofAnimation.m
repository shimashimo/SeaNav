function fig = SixDOFanimation(varargin)
    ...
    % Create figure
    fig = figure('NumberTitle', 'off', 'Name', 'Simulation');
    if(FullScreen)
    ...
    quivXhandle = quiver3(ox, oy, oz, ux, vx, wx,  'r', 'ShowArrowHead', ShowArrowHeadStr, 'MaxHeadSize', 0.999999, 'AutoScale', 'on', 'AutoScaleFactor', 0.7);
    quivYhandle = quiver3(ox, oy, oz, uy, vy, wy,  'g', 'ShowArrowHead', ShowArrowHeadStr, 'MaxHeadSize', 0.999999, 'AutoScale', 'on', 'AutoScaleFactor', 0.7);
    quivZhandle = quiver3(ox, ox, oz, uz, vz, wz,  'b', 'ShowArrowHead', ShowArrowHeadStr, 'MaxHeadSize', 0.999999, 'AutoScale', 'on', 'AutoScaleFactor', 0.7);
    ...
    for i = 1:numPlotSamples
        ...
        % Update graph title
        if(strcmp(Title, ''))
            titleText = sprintf('Data %i of %i', 1+((i-1)*SamplePlotFreq), numSamples);
        else
            titleText = strcat(Title, ' (', sprintf('Data %i of %i', 1+((i-1)*SamplePlotFreq), numSamples), ')');
        end
        title(titleText);
